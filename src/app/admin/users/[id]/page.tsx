  ...
  return (
    <>
      <AdminHeader
        ...
        description="Operational account metadata and recent activity. User-submitted content is intentionally excluded."
      />
      <PrivacyNotice />
      <div className={styles.detailGrid}>
        <AdminPanel title="Account">
          <dl className={styles.detail}>
            <div>
              <dt>Identity</dt>
              <dd>{profile.email || profile.phone || "Verified user"}</dd>
            </div>
            <div>
            ...
            <div>
              <dt>Plan</dt>
              <dd>
                <StatusBadge value={profile.plan} />
              </dd>
            </div>
            <div>
              <dt>Role</dt>
            ...
            <div>
              <dt>Last active</dt>
              <dd>{formatDate(profile.last_active_at)}</dd>
            </div>
            <div>
              <dt>Usage</dt>
              <dd>
        ...
        <AdminPanel title="Subscription">
          <dl className={styles.detail}>
            <div>
              <dt>Status</dt>
              <dd>
                <StatusBadge value={subscription?.status || "inactive"} />
              </dd>
            </div>
            <div>
              <dt>Current plan</dt>
        ...
        <AdminPanel
          title="Access role"
          description="Role changes are server-authorized and written to the audit log."
        >
          <form action={updateUserRole} className={styles.rule}>
      ...
      <AdminPanel title="Recent fact-check metadata">
        {checks.length ? (
          <AdminTable>
            <thead>
              <tr>
            ...
            <tbody>
              {checks.map((check) => (
                <tr key={check.id}>
                  ...
                  <td>{check.input_type}</td>
                  <td>{check.category}</td>
                  <td>
                    <StatusBadge value={check.verdict} />
                  </td>
                  <td>{check.truth_score ?? "Not scored"}</td>
            ...
            <tbody>
              {errors.map((error) => (
                <tr key={error.id}>
                  ...
                  <td>{error.message}</td>
                  <td>{error.endpoint || error.page || "—"}</td>
                </tr>
              ))}
            </tbody>
          </AdminTable>
        ) : (
          <EmptyAdminState title="No errors for this user" />
        )}
      </AdminPanel>
    </>
  );
