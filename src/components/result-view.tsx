import { AlertTriangle, BookOpenCheck, CircleHelp, Lightbulb, ListChecks, ShieldCheck } from "lucide-react";
    ...
    <>
      <section className="panel">
        <div className={styles.hero}>
          <div><span className="status-pill">{result.verdict} · {result.category}</span><h1>{result.summary}</h1><p>{result.claimType} · Assessed {new Date(check.createdAt).toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" })}</p></div>
