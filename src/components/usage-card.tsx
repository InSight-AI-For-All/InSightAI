...
export function UsageCard({ plan, used, remaining }: { plan: Plan; used: number; remaining: number }) {
  ...
  return (
    <article className="panel metric-card">
      ...
      <p className="muted" style={{ margin: "10px 0 0", fontSize: ".78rem" }}>{remaining.toLocaleString()} checks remaining {plan.cadence === "month" ? "this month" : ""} {plan.id === "free" && <>· <Link href="/pricing" style={{ color: "var(--success-green)", fontWeight: 800 }}>Upgrade</Link></>}</p>
    </article>
  );
}